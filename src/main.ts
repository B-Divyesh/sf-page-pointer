...
import { addSession, clearData, exportData, getPreferences, importData, savePreferences, type Preferences } from './storage';
...
const legalRoute = location.pathname.replace(/\/$/, '');
if (legalRoute === '/privacy' || legalRoute === '/terms') renderLegal(legalRoute);
else renderHome();
...
function renderLegal(route: string): void {
  ...
  app.innerHTML = `
    <header class="site-header compact"><a class="brand" href="/" aria-label="Page Pointer home"><img src="/assets/mark.svg" alt="" width="36" height="36"><span>Page Pointer</span></a></header>
    <main id="main" class="legal-page">
      <p class="eyebrow">Sheet ${privacy ? 'P-01' : 'T-01'} · effective 28 August 2026</p>
      <h1>${privacy ? 'Privacy, drawn plainly.' : 'Fair terms, no fine-print maze.'}</h1>
      ${privacy ? `
        <p class="lede">Your camera frames stay on your device. Page Pointer does not send, record, or save pictures of books.</p>
        <h2>What stays on this device</h2><p>Your guide preferences and up to 50 brief session summaries (start time, duration, and camera/demo source) are stored in IndexedDB. A purchased license token and its last verification result are stored in localStorage. You can export or erase local reading data from Settings.</p>
    ...
    <footer><p>© 2026 Sociobot · <a href="/privacy">Privacy</a> · <a href="/terms">Terms</a></p></footer>`;
}

function renderHome(): void {
  app.innerHTML = `
    <header class="site-header">
      <a class="brand" href="#top" aria-label="Page Pointer home"><img src="/assets/mark.svg" alt="" width="36" height="36"><span>Page Pointer</span></a>
    ...
    <main id="main">
      <section class="hero" id="top">
        <div class="hero-copy">
          ...
          <h1>Keep their eyes<br><em>with the story.</em></h1>
          <p class="lede">Aim your phone at a physical book, tap the word being read, and leave the pointing to a calm, high-contrast guide.</p>
          <div class="hero-actions">
            <button class="primary-button" id="open-camera" type="button"><span>Open camera</span><span aria-hidden="true">↗</span></button>
            <button class="secondary-button" id="try-demo" type="button">Try without a camera</button>
          </div>
          <p class="privacy-note"><span aria-hidden="true">◎</span> Frames stay on this device. No account. No OCR archive.</p>
    ...
    <footer><div class="brand footer-brand"><img src="/assets/mark.svg" alt="" width="32" height="32"><span>Page Pointer</span></div><p>A small, private instrument for shared reading.</p><nav aria-label="Legal"><a href="/privacy">Privacy</a><a href="/terms">Terms</a></nav><p class="generated-note">Hero artwork generated originally for Page Pointer.</p></footer>`;
  void initialiseHome();
}
...
async function initialiseHome(): Promise<void> {
  ...
  let trackingTimer = 0;
  let supporterUnlocked = cachedUnlock();
  let timerInterval = 0;
  let timerStarted = 0;
  ...
  async function openCamera(): Promise<void> {
    ...
    if (!navigator.mediaDevices?.getUserMedia) {
      cameraMessage.innerHTML = '<strong>This browser cannot open a camera.</strong> Use “Try without a camera,” or open Page Pointer in a current mobile browser.';
      return;
    }
    try {
      ...
      cameraMessage.innerHTML = denied
        ? '<strong>Camera access is off.</strong> Allow it in your browser’s site settings, then try again—or use the no-camera demo.'
        : '<strong>The rear camera did not open.</strong> Close other camera apps and try again, or use the no-camera demo.';
    }
  }
  ...
  byId<HTMLButtonElement>('try-demo').addEventListener('click', () => startWorkspace('demo'));
  ...
  viewfinder.addEventListener('keydown', (event) => {
    if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); placeAt(); }
  });
